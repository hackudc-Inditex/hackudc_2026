"""
PIPELINE CON ENTRENAMIENTO - El definitivo.

Aprende una proyección que mapea embeddings de crops de bundle al espacio de
embeddings de producto. Esto resuelve el problema principal: CLIP genérico no
entiende la relación entre fotos promocionales (bundles) y fotos de catálogo
(productos con fondo blanco).

Fases:
  1. Preparación de datos y metadatos
  2. YOLO: detectar prendas en todos los bundles de training
  3. CLIP: generar embeddings (productos, crops, bundles)
  4. ENTRENAMIENTO: aprender proyección crop→product space con contrastive loss
  5. VALIDACIÓN: medir recall@15 en hold-out
  6. INFERENCIA: combinar modelo entrenado + metadatos → submission

Uso:
  pip install open-clip-torch faiss-cpu ultralytics huggingface_hub torch
...
def main():
    ...
    # Train/val split (80/20 by bundle)
    all_train_bids = list(train_bundle_products.keys())
    random.seed(42)
    random.shuffle(all_train_bids)
    split = int(0.8 * len(all_train_bids))
    train_bids = all_train_bids[:split]
    val_bids = all_train_bids[split:]
    print(f"  Train bundles: {len(train_bids)}, Val bundles: {len(val_bids)}")
    ...
    # Bundle embeddings (ALL training bundles)
    ...
    crops_cache_path = EMBEDDINGS_DIR / f"crops_data_{model_tag}.pkl"
    ...
    model_save_path = MODELS_DIR / f"projection_{model_tag}.pt"
    ...
    if SKIP_TRAIN and model_save_path.exists():
        ...
        print(f"\n[FASE 5] Entrenando projection head ({NUM_EPOCHS} epochs)...")
    ...
    # Save
    out_path = SUBMISSIONS_DIR / f"submission_trained_{model_tag}.csv"
